...
public class SkinTotemCommand {
	...
	public static LiteralArgumentBuilder<CommandSourceStack> getInfoCommand() {
		return literal("info").executes(ctx -> {
	...
	public static LiteralArgumentBuilder<CommandSourceStack> getCreditsCommand() {
		return literal("credits").executes(ctx -> {
			ctx.getSource().sendSuccess(() -> Component.literal(
				"\n§6╔═══════════════════════════════════╗\n" +
				"§6║  §bSkinTotem §fv1.0.0               §6║\n" +
	...
	public static LiteralArgumentBuilder<CommandSourceStack> getTlCommand() {
		return literal("tl")
			.then(argument("nickname", StringArgumentType.word())
				.executes(ctx -> renameHeldTotem(ctx, "#" + StringArgumentType.getString(ctx, "nickname"))));
	}

	public static LiteralArgumentBuilder<CommandSourceStack> getElyCommand() {
		return literal("ely")
			.then(argument("nickname", StringArgumentType.word())
				.executes(ctx -> renameHeldTotem(ctx, "@" + StringArgumentType.getString(ctx, "nickname"))));
	}

	public static LiteralArgumentBuilder<CommandSourceStack> getUrlCommand() {
		return literal("url")
			.then(argument("link", StringArgumentType.greedyString())
				.executes(ctx -> renameHeldTotem(ctx, "url:" + StringArgumentType.getString(ctx, "link"))));
	}

	public static LiteralArgumentBuilder<CommandSourceStack> getMojangCommand() {
		return literal("mojang").executes(ctx -> {
			MojangAPI.useFallbackAPI = true;
			ctx.getSource().sendSuccess(() -> Component.literal(P + "§aMojang refresh enabled (using fallback API)"), false);
			return 1;
		});
	}

	public static LiteralArgumentBuilder<CommandSourceStack> getModelCommand() {
		return literal("model")
			.then(argument("model_id", StringArgumentType.word())
				.suggests((ctx, builder) -> {
					String[] models = {"2d_doll", "3d_doll", "3d_funko", "gnom", "mini_3d", "parrot", "player_bucket", "pots", "rat", "stairs", "wheelchair"};
				...
				.executes(ctx -> {
					...
					com.darkz.skintotem.config.SkinTotemConfig.getInstance().setStandardTotemDollModelValue(com.darkz.skintotem.SkinTotem.id("dolls/" + modelId + ".bbmodel"));
					com.darkz.skintotem.config.SkinTotemConfig.getInstance().save();
					ctx.getSource().sendSuccess(() -> Component.literal(P + "§aDefault model set to: §f" + modelId), false);
					return 1;
				}));
	}

	private static int renameHeldTotem(com.mojang.brigadier.context.CommandContext<CommandSourceStack> ctx, String name) {
		...
		if (stack.isEmpty() || stack.getItem() != Items.TOTEM_OF_UNDYING) {
			ctx.getSource().sendFailure(Component.literal("§cHold a Totem of Undying in your main hand!"));
			return 0;
		}

		stack.setHoverName(Component.literal(name));

		ctx.getSource().sendSuccess(() -> Component.literal(P + "§aSkin totem set to: §f" + name), false);
		return 1;
	}

	public static com.mojang.brigadier.Command<CommandSourceStack> getHelpExecutor() {
		return ctx -> {
			ctx.getSource().sendSuccess(() -> Component.literal(
				P + "§7Commands:\n" +
				"  §f/skintotem info\n" +
