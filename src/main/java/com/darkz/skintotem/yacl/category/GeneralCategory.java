...
public class GeneralCategory {
	...
	private static OptionGroup getMainGroup(SkinTotemModConfig defConfig, SkinTotemModConfig config) {
	...
	public static OptionGroup getThreadGroup(SkinTotemModConfig defConfig, SkinTotemModConfig config) {
				...
				.options(
						SimpleOption.<Integer>startBuilder("parallel_tasks_count")
								.withBinding(defConfig.getParallelTasksCount(), config::getParallelTasksCount, (i) -> {
									...
									SkinTotemModTaskExecutor.reload();
								}, false)
								.withDescription(SimpleContent.NONE)
